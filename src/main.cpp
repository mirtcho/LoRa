...
sf_t sf =SF11; //mma  SF7;

// Set center frequency
uint32_t  freq =867500000; //=867.5
...
size_t LoRa_write(const uint8_t *buffer, size_t size)
{
  ...
  printf("current lenght=%d        fifo [tr=%x\n",currentLength,readRegister(REG_FIFO_ADDR_PTR));
  // check size
...
void  transmit_packet (int msg_nr)
{
   BeginPacket();
   //LoRa.print("hello ");
   //LoRa.print(counter);
   LoRa_write ( (uint8_t *)"hello 1234",10);      
   EndPacket();
}
...
int main () {
    ...
    SetupLoRa();
    LoRa_setTxPower(7,PA_OUTPUT_RFO_PIN+1);     //pa_boost_pin
